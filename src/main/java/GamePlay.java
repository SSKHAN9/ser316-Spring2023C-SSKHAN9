...
public class GamePlay implements GamePlayInterface {
    ...
    public boolean removeOpponent(Character opponent) {
        ...
        for (int o = 0; o < opponents.size(); o++) {
            if (opponents.get(o).getClass().getName().equals(opponent.getClass().getName())) {
                opponents.remove(o);
                removed = true;
            }
        }
     ...
     * Experience is only gained if the character still has
    ...
    /**
     * Function to add experience points for
     * attacked character as well as update their
     * health based on the attack and
     * their protection.
     * If their protection is higher than the
     * blowDamage then the character will heal by half of
     * that difference they will also gain
     * the full difference as experience

     * If their protection is lower or equal than the blowDamage
     * then the character will take half the difference as experience
     * and the health will be reduced
     ...
     * @param blowDamage full force of attack without protection
                 factored in
    ...
    /**
     * Function that facilitates the attacker dealing damage to
     * their opponent and then the opposite.

     * A character can only attack if both still have health
     * greater than 0, this needs to be true for both attacks happening here

     * You do NOT have the implemented methods for this but just 5
     * implemented versions in the .class files in the cls
     * directory. So you need to Blackbox test this method
     * based on the description you get here. As you see the method returns void,
     ...
     * which you should BlackBox test first.

     * An attack only happens if health>0 for both characters
     * The first character attacks first, by using dealsDamage
     * and the opponent takesDamage.
    ...
    /**
     * This method returns the amount of experience points earned
     * by the player during one round of attacks.
     ...
     * - The player will just iterate through the list of opponents in the order
     * they are in
     * - The attack from an opponent and the attack on the same opponent
     ...
     *      rounded up to the next integer in experience points. Your player then moves onto
     *      the next opponent.
     * - The attack and levelUp are in separate methods (for white box testing in assignment 3
     * you can assume that these methods work
